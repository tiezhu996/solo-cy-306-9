...
- 消息通知：报名成功、审核结果、签到成功自动通知；已发布活动的标题、开始/结束时间、地点、名额被编辑保存后，自动向每位有效报名者（未取消、审核未被拒绝，含待审核/已通过/已签到）合并发送一条「活动变更」未读通知，正文列出各字段旧值与新值；草稿或已结束活动保存、仅修改描述、封面、类型、报名截止等无关字段不发送。
